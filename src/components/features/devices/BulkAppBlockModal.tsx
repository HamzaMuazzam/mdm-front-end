...
import { AppWindow, X, Loader2, Search, Check, CheckSquare, Square, Users, Plus, Trash2, Shield, ShieldOff } from 'lucide-react';
import type { Device } from '@/types/device.types';
import { useBulkAppBlock } from '@/hooks/useDevices';
import { toast } from '@/hooks/useToast';

/**
 * Bulk block/unblock applications (by package id) on one or many devices at once.
...
export function BulkAppBlockModal({ devices, onClose }: { devices: Device[]; onClose: () => void }) {
  ...
  const addPackages = () => {
    ...
    setPackageIds((prev) => Array.from(new Set([...prev, ...parsed])));
    setPackageInput('');
  };

  const removePackage = (pkg: string) => setPackageIds((prev) => prev.filter((p) => p !== pkg));

  const canApply = selected.size > 0 && packageIds.length > 0 && !bulkMutation.isPending;
      ...
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-5 pt-5 pb-4 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Block or unblock apps by package id on multiple devices</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex flex-col sm:flex-row flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex flex-col flex-1 min-h-0">
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
              <div>
                <p className="text-xs font-semibold text-gray-500 mb-2">ACTION</p>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 mb-2">APP PACKAGE IDS</p>
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder="com.android.chrome, com.whatsapp…"
                    value={packageInput}
                ...
                <p className="mt-1.5 text-[11px] text-gray-400">
                  Paste one or many package ids (separated by commas, spaces, or new lines). Apps not installed on a device are skipped.
                </p>
              </div>

              {packageIds.length === 0 ? (
                <p className="text-xs text-gray-400">Add one or more app package ids to {mode}.</p>
              ) : (
                <div className="space-y-1.5">
                  {packageIds.map((pkg) => (
                    <div key={pkg} className="flex items-center gap-2 rounded-md border border-gray-200 px-3 py-2">
                      <p className="flex-1 min-w-0 text-xs font-mono text-gray-800 truncate">{pkg}</p>
                      <button
                        type="button"
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-4 shrink-0">
          ...
          >
            ...
            {mode === 'block' ? 'Block' : 'Unblock'} {packageIds.length} app{packageIds.length !== 1 ? 's' : ''} on {selected.size} device{selected.size !== 1 ? 's' : ''}
          </button>
        </div>
      </div>
    </div>

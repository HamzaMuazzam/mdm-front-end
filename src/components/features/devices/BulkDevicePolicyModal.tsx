...
import { ShieldAlert, X, Loader2, Search, Check, CheckSquare, Square, Users } from 'lucide-react';
...
/** Bulk "Update & Security Policy" editor — select many devices and apply one policy. */
export function BulkDevicePolicyModal({ devices, onClose }: { devices: Device[]; onClose: () => void }) {
  ...
  const toggleAll = () => {
    ...
    else setSelected(new Set(filtered.map((d) => d.deviceUuid)));
  };
      ...
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-5 pt-5 pb-4 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-semibold text-gray-900">Bulk Update &amp; Security Policy</h2>
              <p className="text-xs text-muted-foreground">Apply the same policy to multiple devices at once</p>
            </div>
          </div>
          <button
            type="button"
